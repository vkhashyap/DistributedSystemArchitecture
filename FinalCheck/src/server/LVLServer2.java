...
public class LVLServer2 extends ICenterServerPOA implements Runnable {
	...
	public LVLServer2() throws Exception {
		super();
	}

	public void initializeLogger() {
		lvlLogger = new LogManager("lvl2");
	...
	public static void togglePrimary() throws Exception {
		System.out.println("Primary is changed to lvlserver2");
		isPrimary = !isPrimary;
...
//			threadSocket.start();
			...
			HeartBeat heartbeatToBackUp2 = new HeartBeat(ServerFrontEnd.portLaval2, ServerFrontEnd.portLaval1);
			heartbeatToBackUp2.startUp();

			HeartBeat heartbeatToBackUp3 = new HeartBeat(ServerFrontEnd.portLaval2, ServerFrontEnd.portLaval3);
			heartbeatToBackUp3.startUp();

		} else {

			// listening to heartbeat from the primary server
			Thread heartbeat = new Thread(new HeartBeat(5002));
			heartbeat.start();

			// check for primary's failure
			Thread t = new Thread(new FailureDetector(5002));
			t.start();
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (isPrimary) {
			...
			Thread threadSocket = new Thread(new LVLServer2());
			threadSocket.start();
		...
		try {
			...
			NameComponent path[] = ncRef.to_name(ManagerClient.LVL_SERVER_2);
	...
	private void broadcast(String request) {
		new ServerMultiCastSender(5002, ServerFrontEnd.lvlHostAddress, request).start();
	}
