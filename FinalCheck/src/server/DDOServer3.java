...
public class DDOServer3 extends ICenterServerPOA implements Runnable {
	private ORB orbDDO;
	...
	public DDOServer3() throws Exception {
		super();
	...
	public static void togglePrimary() throws Exception {
		System.out.println("Primary is changed to ddoserver3");
		isPrimary = !isPrimary;
...
//			threadSocket.start();
			...
			HeartBeat heartbeatToBackUp2 = new HeartBeat(ServerFrontEnd.portDollard3, ServerFrontEnd.portDollard1);
			heartbeatToBackUp2.startUp();

			HeartBeat heartbeatToBackUp3 = new HeartBeat(ServerFrontEnd.portDollard3, ServerFrontEnd.portDollard2);
			heartbeatToBackUp3.startUp();

		} else {

			// listening to heartbeat from the primary server
			Thread heartbeat = new Thread(new HeartBeat(6003));
			heartbeat.start();

			// check for primary's failure
			Thread t = new Thread(new FailureDetector(6003));
			t.start();
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (isPrimary) {
			...
			Thread threadSocket = new Thread(new DDOServer3());
			threadSocket.start();
		...
		try {
			...
			NameComponent path[] = ncRef.to_name(ManagerClient.DDO_SERVER_3);
	...
	private void broadcast(String request) {
		new ServerMultiCastSender(6003, ServerFrontEnd.ddoHostAddress, request).start();
	}
