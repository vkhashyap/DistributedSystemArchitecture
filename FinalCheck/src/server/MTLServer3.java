...
public class MTLServer3 extends ICenterServerPOA implements Runnable {
	...
	public static boolean isPrimary = true;
	public static long lastHeartBeatTime3 = System.currentTimeMillis();

	public HeartBeat heartbeatToBackUp2;
	public HeartBeat heartbeatToBackUp3;

	public MTLServer3() throws Exception {
		super();
		// Thread threadSocket = new Thread(new MTLServer3());
		// threadSocket.start();

	}

	public void initializeLogger() {
		mtlLogger = new LogManager("mtl3");
	...
	public static void togglePrimary() throws Exception {
		System.out.println("Primary is changed to mtlserver3");
		isPrimary = !isPrimary;
		...
		if (isPrimary) {
			...
			HeartBeat heartbeatToBackUp2 = new HeartBeat(ServerFrontEnd.portMontreal3, ServerFrontEnd.portMontreal2);
			heartbeatToBackUp2.startUp();

			HeartBeat heartbeatToBackUp3 = new HeartBeat(ServerFrontEnd.portMontreal3, ServerFrontEnd.portMontreal1);
			heartbeatToBackUp3.startUp();

		} else {

			// listening to heartbeat from the primary server
			Thread heartbeat = new Thread(new HeartBeat(4003));
			heartbeat.start();

			// check for primary's failure
			Thread t = new Thread(new FailureDetector(4003));
			t.start();
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (isPrimary) {
			...
			mtl.heartbeatToBackUp2 = new HeartBeat(4113, 4112);
			mtl.heartbeatToBackUp2.startUp();

			mtl.heartbeatToBackUp3 = new HeartBeat(4113, 4111);
			mtl.heartbeatToBackUp3.startUp();

		} else {

			// listening to heartbeat from the primary server
			Thread heartbeat = new Thread(new HeartBeat(4113));
			heartbeat.start();

			// check for primary's failure
			Thread t = new Thread(new FailureDetector(4113));
			t.start();
		}
		...
		try {
			...
			NameComponent path[] = ncRef.to_name(ManagerClient.MTL_SERVER_3);
	...
	private void broadcast(String request) {
		new ServerMultiCastSender(4003, ServerFrontEnd.mtlHostAddress, request).start();
	}

	public void killMyprimaryServer(String loc) throws InterruptedException {
	...
	public void killThreads() {
		...
		heartbeatToBackUp2.shutDown();
		heartbeatToBackUp3.shutDown();
	}
}
