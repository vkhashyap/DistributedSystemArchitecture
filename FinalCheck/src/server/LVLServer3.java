...
public class LVLServer3 extends ICenterServerPOA implements Runnable {
	...
	public LVLServer3() throws Exception {
		super();
		// HeartBeat objHeartBeatBackup1 = new HeartBeat(ServerFrontEnd.portLaval3,
		// ServerFrontEnd.portLaval1);
		// HeartBeat objHeartBeatBackup2 = new HeartBeat(ServerFrontEnd.portLaval3,
		// ServerFrontEnd.portLaval2);
		// objHeartBeatBackup1.startUp();
		// objHeartBeatBackup2.startUp();
	}

	public void initializeLogger() {
		lvlLogger = new LogManager("lvl3");
	...
	public static void togglePrimary() throws Exception {
		System.out.println("Primary is changed to lvlserver3");
		isPrimary = !isPrimary;
...
//			threadSocket.start();
			...
			HeartBeat heartbeatToBackUp2 = new HeartBeat(ServerFrontEnd.portLaval3, ServerFrontEnd.portLaval1);
			heartbeatToBackUp2.startUp();

			HeartBeat heartbeatToBackUp3 = new HeartBeat(ServerFrontEnd.portLaval3, ServerFrontEnd.portLaval2);
			heartbeatToBackUp3.startUp();

		} else {

			// listening to heartbeat from the primary server
			Thread heartbeat = new Thread(new HeartBeat(5003));
			heartbeat.start();

			// check for primary's failure
			Thread t = new Thread(new FailureDetector(5003));
			t.start();
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (isPrimary) {
			...
			Thread threadSocket = new Thread(new LVLServer3());
			threadSocket.start();
		...
		try {
			...
			NameComponent path[] = ncRef.to_name(ManagerClient.LVL_SERVER_3);
	...
	private void broadcast(String request) {
		new ServerMultiCastSender(5003, ServerFrontEnd.lvlHostAddress, request).start();
	}
