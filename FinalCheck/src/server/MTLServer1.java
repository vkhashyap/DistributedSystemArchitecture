...
public class MTLServer1 extends ICenterServerPOA implements Runnable {
	...
	public MTLServer1() throws Exception {
		super();
	}

	public void initializeLogger() {
		mtlLogger = new LogManager("mtl1");
	...
	public static void togglePrimary() throws Exception {
		System.out.println("Primary is changed to mtlserver1");
		isPrimary = !isPrimary;
...
//			threadSocket.start();
			...
			HeartBeat heartbeatToBackUp2 = new HeartBeat(ServerFrontEnd.portMontreal1, ServerFrontEnd.portMontreal2);
			heartbeatToBackUp2.startUp();

			HeartBeat heartbeatToBackUp3 = new HeartBeat(ServerFrontEnd.portMontreal1, ServerFrontEnd.portMontreal3);
			heartbeatToBackUp3.startUp();

		} else {

			// listening to heartbeat from the primary server
			Thread heartbeat = new Thread(new HeartBeat(4001));
			heartbeat.start();

			// check for primary's failure
			Thread t = new Thread(new FailureDetector(4001));
			t.start();
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (isPrimary) {
			...
			HeartBeat heartbeatToBackUp2 = new HeartBeat(4111, 4112);
			heartbeatToBackUp2.startUp();

			HeartBeat heartbeatToBackUp3 = new HeartBeat(4111, 4113);
			heartbeatToBackUp3.startUp();

		} else {

			// listening to heartbeat from the primary server
			Thread heartbeat = new Thread(new HeartBeat(4111));
			heartbeat.start();

			// check for primary's failure
			Thread t = new Thread(new FailureDetector(4111));
			t.start();
		}
		...
		try {
			...
			NameComponent path[] = ncRef.to_name(ManagerClient.MTL_SERVER_1);
	...
	private void broadcast(String request) {
		new ServerMultiCastSender(4001, ServerFrontEnd.mtlHostAddress, request).start();
	}
