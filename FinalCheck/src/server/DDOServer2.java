...
public class DDOServer2 extends ICenterServerPOA implements Runnable {
	private ORB orbDDO;
	...
	public DDOServer2() throws Exception {
		super();
	...
	public static void togglePrimary() throws Exception {
		...
		//Set values for new primary
		
		
		System.out.println("Primary is changed to ddoserver2");
		isPrimary = !isPrimary;
...
//			threadSocket.start();
			...
			HeartBeat heartbeatToBackUp2 = new HeartBeat(ServerFrontEnd.portDollard2, ServerFrontEnd.portDollard1);
			heartbeatToBackUp2.startUp();

			HeartBeat heartbeatToBackUp3 = new HeartBeat(ServerFrontEnd.portDollard2, ServerFrontEnd.portDollard3);
			heartbeatToBackUp3.startUp();

		} else {

			// listening to heartbeat from the primary server
			Thread heartbeat = new Thread(new HeartBeat(6002));
			heartbeat.start();

			// check for primary's failure
			Thread t = new Thread(new FailureDetector(6002));
			t.start();
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (isPrimary) {
			...
			Thread threadSocket = new Thread(new DDOServer2());
			threadSocket.start();
		...
		try {
			...
			NameComponent path[] = ncRef.to_name(ManagerClient.DDO_SERVER_2);
	...
	private void broadcast(String request) {
		new ServerMultiCastSender(6002, ServerFrontEnd.ddoHostAddress, request).start();
	}
